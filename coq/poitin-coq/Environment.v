...
Hint Local Extern 1 (~ In _ _) => simpl_env in *; decideFSet.
...
Lemma ok_map : forall E (f : A -> A),
  ok E -> ok (map f E).
Proof.
  intros.
  rewrite_env (map f E ++ nil).
  eapply ok_map_app_l.
  simpl_env.
  auto.
Qed.
...
Proof with auto.
  induction F as [|(y,b)]; intros x c Ok; simpl_env in *.
    ...
    inversion Ok; subst. simpl_env in *.
      apply notin_union. notin_solve. apply (IHF _ _ H1).
Qed.
...
Proof with auto.
  ...
  destruct (eq_atom_dec x y)... intros _ J. destruct J. decideFSet.
Qed.
...
Proof with auto.
  ...
  inversion Ok; subst. destruct (eq_atom_dec x z)...
    destruct H3. simpl_env. decideFSet.
Qed.
...
Proof with auto.
  ...
  inversion Ok; subst. destruct (eq_atom_dec z x)...
    destruct H4. simpl_env. decideFSet.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H; subst; auto using mem_1, mem_2, union_2, union_3, singleton_2. AtomSetDecide.fsetdec.
Qed.
...
Proof.
  ...
  binds_cases H.
  binds_cases H. intuition. decideFSet.
Qed.
