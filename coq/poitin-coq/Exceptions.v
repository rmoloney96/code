...
Definition ret_exn (A : Type) (a : A) := no_exn A a.

Program Instance Monad ExceptionM := 
  ret := ret_exn ;
  bind := bind_exn.

  Next Obligation. (* bind left ret *)
  Proof. 
    unfold bind_exn ; unfold ret_exn ; destruct x ; auto.
  Qed.

  Next Obligation. (* bind right ret *) 
  Proof. 
    unfold bind_exn ; unfold ret_exn ; destruct x ; auto.
  Qed.
...
Definition fail_test (n:nat) := 
  m <- (P n) ;; ret (m,"no prob")
  |:| zero_exn, msg => ret (n,msg)
  |:| syntax_error, msg => ret (0,msg).
