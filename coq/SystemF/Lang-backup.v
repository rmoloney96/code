...
Proof.
  ...
  rewrite IHty.
  rewrite tyshift_comm. auto. omega.  omega.
Qed.
...
Theorem nth_tysub_tyshift : forall l n ty ty0,
  nth n (map (tyshift 0) l) Zero = ty -> 
  nth n l Zero = tysub ty 0 ty0.
Proof.
  induction l.
  intros. simpl in *. case n.

Theorem tysub_preservation : forall t n l ty0 ty1, 
  Derivation (n; l |= TApp (Lam t) ty0 @ tysub ty1 0 ty0) -> 
  Derivation (n; l |= tysubt t 0 ty0 @ tysub ty1 0 ty0).
Proof.
  induction t ; intros.
  ...
  inversion H0 ; subst.
  simpl. 
  apply VarIntro ; inversion H8 ; subst. 
  inversion H2 ; subst ; rewrite map_length in H5. auto. 
  inversion H2.
  subst.
  ...
  inversion H8. subst. 
  inversion H2. subst. 
  apply VarIntro. rewrite map_length in H5. auto.
  rewrite H6.
  destruct ty1. simpl in *. numerical.
  intros.
