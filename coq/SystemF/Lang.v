...
(* Counter 
  ...
  XP3 : n0 <= S n
  ============================
   tysub ty0 m ty1 = tysub (tyshiftn n0 0 ty0) (m + n0) (tyshiftn n0 0 ty1)
...
(*
Lemma tysub_reorder : forall (ty ty0 ty1 : Ty) m n, 
  tysub (tysub ty (S (m+n)) (tyshiftn (S n) 0 ty1)) n (tyshiftn n 0 (tysub ty0 m ty1)) = 
  tysub (tysub ty n (tyshiftn n 0 ty0)) (m+n) (tyshiftn n 0 ty1).
Proof.
  refine 
    (fix tysub_reorder (ty ty0 ty1 : Ty) (m n : nat) {struct ty} : 
      tysub (tysub ty (S (m+n)) (tyshiftn (S n) 0 ty1)) n (tyshiftn n 0 (tysub ty0 m ty1)) = 
      tysub (tysub ty n (tyshiftn n 0 ty0)) (m+n) (tyshiftn n 0 ty1) := 
      (match ty as ty' return (ty = ty' -> 
        tysub (tysub ty (S (m+n)) (tyshiftn (S n) 0 ty1)) n (tyshiftn n 0 (tysub ty0 m ty1)) = 
        tysub (tysub ty n (tyshiftn n 0 ty0)) (m+n) (tyshiftn n 0 ty1)) 
         with 
         | TV n => _ 
         | Imp tya tyb => _
         | All tya => _
       end) (refl_equal ty)) ; intros ; subst.

  simpl. 
  destruct n0 ; simpl. numerical.
  subst. 
  rewrite tyshiftn_z_id. rewrite tyshiftn_z_id. rewrite plus_comm. simpl. auto.
  rewrite tyshiftn_z_id. auto. 
  
  numerical. subst.   
  rewrite tysub_into_hole. auto. firstorder.
  subst.
  clear XP2. clear XP3. clear XP1. clear XP.
  induction ty0.
  simpl. numerical.
  destruct n ; numerical. simpl. firstorder.
  simpl. 
  rewrite IHty0_1. rewrite IHty0_2. auto.
  simpl.  
  

  destruct n0. 
  rewrite tysub_into_hole. simpl.
  Focus 2. firstorder. firstorder. 

  induction ty0. simpl. numerical. destruct n0 ; numerical.
  simpl. firstorder.

  simpl. rewrite IHty0_1. rewrite IHty0_2. auto. 
 
  simpl.
  rewrite IHty0. 

  numerical.
  destruct n3 ; intros ; try (numerical). 
  simpl. subst. simpl. firstorder.
  simpl. subst. simpl.
  cut (tyshiftn (S n2) 0 (tysub ty3_1 m0 ty4))
     (tyshiftn (S n2) 0 (tysub ty3_2 m0 ty4))

  

  intros. elimtype False. firstorder.
  intros. simpl. numerical.
  
  simpl.
  case_eq (le_lt_dec (m+n0) n). intros.
  case_eq (eq_nat_dec (m+n0) n). simpl.
  intros. 
  case_eq (le_lt_dec n0 (S n)). intros.  
  ...
  case_eq (le_lt_dec (m+n0) 0). intros. simpl.
  case_eq (eq_nat_dec (m+n0) 0). intros. 
  intros. clear H2. apply plus_is_O in e. inversion e.
  rewrite H3. rewrite tyshiftn_z_id. 
  elimtype False. firstorder.
  intros. numerical. intros. auto.
  simpl. numerical.
Defined.
*) 
