...
Inductive cstep : state -> com -> com -> state -> Prop :=
  | CS_AssStep : forall st i a a',
    ...
    cstep st (i ::= a) (i ::= a') st
  | CS_Ass : forall st i n, 
    cstep st (i ::= (ANum n)) SKIP (update st i n)
  | CS_SeqStep : forall st c1 c1' st' c2,
    cstep st c1 c1' st' ->
    cstep st (c1 ; c2) (c1' ; c2) st'
  | CS_SeqFinish : forall st c2,
    cstep st (SKIP ; c2) c2 st
  | CS_IfTrue : forall st c1 c2,
    cstep st (IFB BTrue THEN c1 ELSE c2 FI) c1 st
  | CS_IfFalse : forall st c1 c2,
    cstep st (IFB BFalse THEN c1 ELSE c2 FI) c2 st
  | CS_IfStep : forall st b b' c1 c2,
    bstep st b b' ->
    cstep st (IFB b THEN c1 ELSE c2 FI) (IFB b' THEN c1 ELSE c2 FI) st
  | CS_While : forall st b c1,
    cstep st (WHILE b DO c1 END) 
             (IFB b THEN (c1; (WHILE b DO c1 END)) ELSE SKIP FI) st
  (* New: *)
  | CS_Par1 : forall st c1 c1' c2 st',
    cstep st c1 c1' st' ->
    cstep st (PAR c1 WITH c2 END) (PAR c1' WITH c2 END) st'
  | CS_Par2 : forall st c1 c2 c2' st',
    cstep st c2 c2' st' ->
    cstep st (PAR c1 WITH c2 END) (PAR c1 WITH c2' END) st'
  | CS_ParDone : forall st,
    cstep st (PAR SKIP WITH SKIP END) SKIP st.

(** A small example: the following program can terminate with X set to
    any value. *)

Inductive refl_step_closure4 {X Y : Type} (R: X -> Y -> Y -> X -> Prop) 
                             : X -> Y -> Y -> X -> Prop :=
  | rsc4_refl  : forall (x : X) (y : Y),
                    refl_step_closure4 R x y y x
  | rsc4_step : forall (x y z : X) (a b c : Y),
                    R x a b y ->
                    refl_step_closure4 R y b c z ->
                    refl_step_closure4 R x a c z.

Definition cstepmany := refl_step_closure4 cstep. 
...
Example par_loop_example_0:
  exists st',
       cstepmany empty_state par_loop SKIP st'
    /\ st' X = 0.
Proof.
  eapply ex_intro. split.
  eapply rsc4_step. apply CS_SeqStep. apply CS_SeqStep. apply CS_Ass. 
  eapply rsc4_step. apply CS_SeqStep. apply CS_SeqFinish.
  eapply rsc4_step. apply CS_SeqStep. apply CS_Ass. 
  eapply rsc4_step. apply CS_SeqFinish.
  eapply rsc4_step. apply CS_Par1. apply CS_Ass.
  eapply rsc4_step. apply CS_Par2. apply CS_While.
  eapply rsc4_step. apply CS_Par2. apply CS_IfStep. 
    apply BS_NotStep. apply BS_Eq1. apply AS_Id.
  eapply rsc4_step. apply CS_Par2. apply CS_IfStep. 
    apply BS_NotStep. apply BS_Eq. simpl. 
  eapply rsc4_step. apply CS_Par2. apply CS_IfStep. 
    apply BS_NotTrue. 
  eapply rsc4_step. apply CS_Par2. apply CS_IfFalse. 
  eapply rsc4_step. apply CS_ParDone. 
  eapply rsc4_refl. 
  reflexivity. Qed.
...
Proof.
  ...
  eapply rsc4_step. apply CS_Par2. apply CS_IfStep. 
    apply BS_NotFalse. 
  eapply rsc4_step. apply CS_Par2. apply CS_IfTrue. 
  eapply rsc4_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. apply AS_Plus1. apply AS_Id.
  eapply rsc4_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_AssStep. apply AS_Plus.  
  eapply rsc4_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_Ass.
  eapply rsc4_step. apply CS_Par2. apply CS_SeqFinish.
...
(** Prove that [par_loop] can terminate with [X] set to any value:
